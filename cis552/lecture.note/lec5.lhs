...
Last time around we defined a type for trees with data at the internal nodes:

> data Tree a = Leaf | Node a (Tree a) (Tree a)

We observed that we could define and use versions of our favorite
higher-order functions for trees:

> treeFold :: (a -> b -> b -> b) -> b -> Tree a -> b
> treeFold _ base Leaf         = base
> treeFold f base (Node a l r) = 
>    f a (treeFold f base l) (treeFold f base r)
...
of `Node`s is equal.  Put another way, we'll only be able to compare
...
> tree1 = Node 3 (Node 2 Leaf Leaf) (Node 1 Leaf Leaf)
> tree2 = Node 3 Leaf Leaf
...
Since it already knows how to compare `Float`s, this one works.
...
library [1].  Predictably, parsing values is a little more complicated
...
creating a new list with the results.  We saw on Monday that
...
[1]  http://hackage.haskell.org/packages/archive/base/latest/doc/html/Prelude.html#t:Read
