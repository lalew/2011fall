...
> twoChar0  = P (\cs -> do (c1,cs')  <- doParse oneChar cs  -- ls :: [(Char, String)]
>                          (c2,cs'') <- doParse oneChar cs'
>                          return ((c1,c2), cs''))
...
> pairP0 p1 p2 = P (\cs -> do (c1,cs')  <- doParse p1 cs 
>                             (c2,cs'') <- doParse p2 cs'
>                             return ((c1,c2), cs''))
...
> p1 `bindP` fp2 = P (\cs -> do (a,cs') <- doParse p1 cs 
>                               doParse (fp2 a) cs')  -- [(b,String)]
...
> manyP p = many1 `chooseP` many0 where
>   many0 = return []
>   many1 = do x <- p
>              xs <- manyP p               
>              return (x:xs)
...
> oneNat0 = do ms <- (string "-") <|> return []
>              xs <- mmany1 digitChar 
>              return $ ((read (ms ++ xs)) :: Integer)
...
> oneNat :: Parser Int
> oneNat = read `liftM` mmanyP digitChar 
...
> sumE :: Parser Int
> sumE = addE <|> prodE 
>   -- addE is the addition of a product expression
>   -- with another sum expression
...
> prodE :: Parser Int
> prodE = mulE <|> factorE
>   -- mulE is the multipliaction of a factor expression
>   -- with another product expression
>   where mulE = do x <- factorE
>                   o <- mulOp                    
>                   y <- prodE 
>                   return $ x `o` y
>
> -- either a parenthesized sum expression, or a number
> factorE :: Parser Int
> factorE = (do x <- char '('
>               y <- sumE                
>               z <- char ')'
