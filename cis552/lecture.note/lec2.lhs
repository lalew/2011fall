...
Write a function that given an argument x, 
and a number n, returns a list containing
n copies of x.
...
Define a function that, given two ints i and j, returns a list
containing all of the numbers at least as big as i but no bigger than
j.
...
This one looks pretty good!  Though we mentioned that one can
also do pattern matching with case statements, as in:
...
datastructure recording which ones pass and fail.
...
meaning that they can be treated just as you would any other
data.

You can pass functions around in *any* manner that you can pass any
other data around in. For example, suppose you have a simple functions
...
are defined in this manner in the standard library.  For example:
